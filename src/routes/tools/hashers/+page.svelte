	...
	const algorithms = [
		{
			...
			label: 'MD5',
			support: 'client',
			note: 'Legacy, fast checksum'
		},
		{
			id: 'sha1',
			label: 'SHA-1',
			support: 'client',
			note: 'Legacy, avoid for security'
		},
		{
			id: 'sha256',
			label: 'SHA-256',
			support: 'webcrypto',
			note: 'Recommended for integrity'
		},
		{
			id: 'sha512',
			label: 'SHA-512',
	...
	const computeHash = async () => {
		...
		try {
			if (selectedAlgorithm === 'md5') {
				outputValue = md5(inputValue);
				return;
			}
			if (selectedAlgorithm === 'sha1') {
				outputValue = sha1(inputValue);
				return;
			}
			if (selectedAlgorithm === 'ripemd160') {
				outputValue = ripemd160(inputValue);
				return;
			}
			...
			outputValue = await hashWithWebCrypto(map[selectedAlgorithm], inputValue);
		} catch (error) {
