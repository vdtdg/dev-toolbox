...
function levenshteinDistance(firstWord, secondWord) {
    ...
    const scoreMatrix = Array(firstWordLength + 1).fill(null).map(() => Array(secondWordLength + 1).fill(null));
    ...
    for (let i = 0; i <= firstWordLength; i++) {
        for (let j = 0; j <= secondWordLength; j++) {
            if (i === 0) {
                ...
                scoreMatrix[i][j] = 1 + Math.min(scoreMatrix[i - 1][j], scoreMatrix[i][j - 1], scoreMatrix[i - 1][j - 1]);
            }
        }
    }
    ...
    return (
        <section className="tool-section">
            ...
            <table className="levenshtein-table">
                ...
                <tr>
                    <td className="column1"><label htmlFor="target">Target String:</label></td>
                    <td className="column2"><input id="target" className="field target-field" value={target} onChange={e => setTarget(e.target.value)}/></td>
                ...
                {inputs.map((input, index) => (
                    <tr key={index}>
                        <td className="column1"><label htmlFor={index.toString()}>Input {index + 1}:</label></td>
                        <td className="column2"><input id={index.toString()} className="field" value={input} onChange={e => handleInputChange(index, e.target.value)}/></td>
                        <td className="column3">{results[index] !== undefined && <span>{results[index]}</span>}</td>
                    </tr>
                ))}
                </tbody>
            </table>
