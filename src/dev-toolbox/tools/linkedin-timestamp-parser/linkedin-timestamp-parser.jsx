...
export default function LinkedinTimestampParser() {
    ...
    return <section className="tool-section">
        ...
        <p>On LinkedIn, posts are displayed with time relative to the current day. This can lead to very vague dates: "1 year ago" can mean a huge
            range of dates. But did you know that the date of the post is hidden in the URL of the post itself?</p>
        <p>Thus, here is a tool to determine the exact LinkedIn post date.</p>
        <p>To find a post URL, click the three dots at the top right of a post, and click "Copy link to post". Then, just paste the URL in the box,
            and click Find Date.</p>
        <label htmlFor="input" className="field-label">LinkedIn Post URL</label>
        <input
            id="input"
            className="field"
            value={input}
        ...
        <div className="action-button-group">
            <button onClick={linkedinTimestampToTime} className="button primary-button action-button">Find Date</button>
        </div>
        <label htmlFor="output" className="field-label">Post date:</label>
        <input
            id="output"
            className="field"
            value={output}
