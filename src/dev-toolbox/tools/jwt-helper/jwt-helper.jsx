...
const JwtHelper = () => {
  ...
  return (
    <section className="tool-section jwt-helper-wrapper">
      <h1 className="tool-title">JWT Helper</h1>
      <p>
        The JWT Helper is a convenient tool designed to facilitate the encoding
        and decoding of JSON Web Tokens (JWTs). You can easily generate a JWT by
        providing a payload and a secret key, or decode an existing JWT to
        verify its contents and signature. This tool is intended to assist
        developers in testing and debugging JWTs during the development of
        secure applications.
      </p>
      ...
      <div className="jwt-helper-footer">
        {error ? (
          ...
          <p className="Signature Verified">Success!</p>
        )}
        <button onClick={handleShareClick}>
          {copied ? "COPIED" : "SHARE JWT"}
        </button>
      </div>
    </section>
  );
